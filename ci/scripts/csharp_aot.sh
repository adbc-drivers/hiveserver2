...
#
# Usage: csharp_aot.sh <workspace> <rid> <lib-name>
#   <rid>       e.g. win-x64, linux-x64, osx-arm64
#   <lib-name>  the published shared library name, e.g.
#               AdbcDrivers.HiveServer2.Native.{dll,so,dylib}
...
# 1. AOT-publish the native shared library.
dotnet publish "${native_project}" -c Release -r "${rid}" -o "${publish_dir}"

# 2. Run the interop tests, pointing them at the freshly published library.
ADBC_TEST_AOT_FIXTURE_PATH="${publish_dir}/${lib_name}" \
