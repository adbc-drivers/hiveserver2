...
namespace AdbcDrivers.Tests.HiveServer2.Hive2
{
    public class DriverTests : Common.DriverTests<ApacheTestConfiguration, HiveServer2TestEnvironment>
    {
        ...
        public override void CanDetectInvalidServer()
        {
            ...
            bool hasHostName = parameters.TryGetValue(HiveServer2Parameters.HostName, out var hostName) && !string.IsNullOrEmpty(hostName);
            if (hasUri)
            {
            ...
            {
                parameters[HiveServer2Parameters.HostName] = "unknownhost.azure.com";
            }
            else
            {
                Assert.Fail($"Unexpected configuration. Must provide '{AdbcOptions.Uri}' or '{HiveServer2Parameters.HostName}'.");
            }
