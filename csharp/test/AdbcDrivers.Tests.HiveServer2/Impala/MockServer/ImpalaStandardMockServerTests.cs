...
{
    /// <summary>
    /// Drive the Impala driver against the TCP + SASL PLAIN + framed-binary
    /// mock. Covers <c>ImpalaStandardConnection</c> + the shared
    /// <c>HiveServer2StandardConnection</c> + SASL plumbing.
    /// </summary>
    [Trait("Category", "MockServer")]
    public class ImpalaStandardMockServerTests
    {
        [Fact]
        public async Task CanExecuteSimpleSelect()
        {
            using var scenario = ImpalaMockServer.CreateStandard();
